---
class: conduit
feature_type: trait
file_basename: Knowledge Domain Piety and Effect
file_dpath: Conduit/1st-Level Features
item_id: knowledge-domain-piety-and-effect
item_index: 08
item_name: Knowledge Domain Piety and Effect
level: 1
scc:
- mcdm.heroes.v1:feature.trait.conduit.1st-level-feature:knowledge-domain-piety-and-effect
scdc:
- 1.1.1:13.1.8.1:08
source: mcdm.heroes.v1
type: feature/trait/conduit/1st-level-feature
---

```ds-ability
name: Knowledge Domain Piety and Effect
metadata:
  class: conduit
  feature_type: trait
  file_basename: Knowledge Domain Piety and Effect
  file_dpath: Conduit/1st-Level Features
  item_id: knowledge-domain-piety-and-effect
  item_index: 8
  item_name: Knowledge Domain Piety and Effect
  level: 1
  scc:
    - mcdm.heroes.v1:feature.trait.conduit.1st-level-feature:knowledge-domain-piety-and-effect
  scdc:
    - 1.1.1:13.1.8.1:08
  source: mcdm.heroes.v1
  type: feature/trait/conduit/1st-level-feature
effects:
  - effect: "- **Piety:** You gain 2 piety the first time in an encounter that the
      Director spends Malice (see *[Draw Steel:
      Monsters](https://mcdm.gg/DS-Monsters)*)."
  - effect: "- **Prayer Effect:** Choose up to five allies within 10 squares of you,
      or choose yourself instead of one ally. Each target gains 1 surge."
```