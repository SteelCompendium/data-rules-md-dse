---
action_type: feature
class: elementalist
feature_type: trait
file_basename: Enchantment of Distance
file_dpath: Elementalist/1st-Level Features/Enchantment
item_id: enchantment-of-distance
item_index: '15'
item_name: Enchantment of Distance
level: 1
scc:
- mcdm.heroes.v1:subfeature.trait.elementalist.1st-level-feature:enchantment-of-distance
scdc:
- 1.1.1:12.1.9.7:15
source: mcdm.heroes.v1
type: subfeature/trait/elementalist/1st-level-feature
---

```ds-feature
type: feature
feature_type: trait
name: Enchantment of Distance
metadata:
  action_type: feature
  class: elementalist
  feature_type: trait
  file_basename: Enchantment of Distance
  file_dpath: Elementalist/1st-Level Features/Enchantment
  item_id: enchantment-of-distance
  item_index: "15"
  item_name: Enchantment of Distance
  level: 1
  scc:
    - mcdm.heroes.v1:subfeature.trait.elementalist.1st-level-feature:enchantment-of-distance
  scdc:
    - 1.1.1:12.1.9.7:15
  source: mcdm.heroes.v1
  type: subfeature/trait/elementalist/1st-level-feature
effects:
  - effect: You have a +2 bonus to the distance of your ranged magic abilities.
```